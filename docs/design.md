...
| 2026-04-04 | Wireframe Maker テキスト自由配置＆インポート改善 | T+ボタンでキャンバス任意位置にテキスト追加（ドラッグ移動・ダブルクリック編集・サイズ/色/太さ変更）、URLインポート時に「既存に追加」「全て置換」選択モーダル追加、Undo/RedoのfreeTexts対応、マルチページfreeTexts同期修正 |
