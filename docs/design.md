...
**事業（サービス）起点への構造再設計（S091）:** 「営業資料インポートだとどの事業で試算しているか分からない／LTV・成約率・客単価・粗利率は事業で激変するのに資料だけで判断するのは欠陥」という指摘を受けた根本再設計。考え方を「**経済性（成約率・客単価・粗利率・LTV）＝事業が持つ／予算・流入＝資料 or 手入力が供給**」に統一。(1) 導線を3ステップに（STEP1 事業を選ぶ → STEP2 予算・流入＝手入力 or 資料インポート → STEP3 判定）。DOM もこの順に並べ替え（`svc-step` で事業選択を起点に置き、`import-step` を STEP2 として下げた）。(2) `applyExtracted` は予算・流入（budget/mode/cpc/clicks）のみ読み取り、**事業が選択済み（`activePresetKey`）なら経済性を上書きしない**（事業未選択時のみ一般目安を仮置き）。(3) `econAnchored`（経済性が事業選択 or 手入力で確定したか）で**判定をゲート**：未確定のうちは KPI・ファネル・判定を「—」にし、verdict は「STEP1 事業を選んでください」（`.verdict.pending`）。事業選択・経済性の手入力・目安ヘルパー・プラン適用で `econAnchored=true`。`isAnchored() = activePresetKey!==null || econAnchored`。verdict 優先順位は 予算未入力 → 事業未選択 → 黒字/赤字。(4) 基準バー（S090）の文言も「<事業>の経済性 × 資料『…』の予算・流入」等に更新。(5) 全体整合レビュー：KPI・ファネル・損益分岐バー（be-target/bar）も `ok`（=hasInput && isAnchored）でゲート。コピー／PDF印刷／プラン比較追加は事業未選択ならトーストで促し実行しない（コピー文面に「事業（前提）」行を追加）。AI 所見パネルは投資判断（GO/REVIEW/NOGO バッジ）を出さない方針に変更（判定は事業×アプリ計算が担う）＝見出しを「AI が資料から読み取った内容＆注意点」に、verdictReason は「補足コメント」として任意表示。各カード（svc-step／import-step／入力／結果）を白地＋ネイビー上アクセントに統一。`setUseCase` のプリセット読込は state を値流し込み前に確定（stale render 防止）。多プラン取込後に事業を選ぶと `syncScenarioEconomics()` で比較表各プランの経済性も選んだ事業に揃える（予算・流入は維持）。

**初心者向け仕上げ＆構造レビュー（S092）:** (1) **レイアウト整列**：ガイド・STEP・入力/結果の幅が不揃い（一部880px・一部コンテナ幅）で右端がガタついていた不具合を、全ブロック同一幅（`.container` 1080px いっぱい）に統一して解消。(2) **事業選択ボタンを大型化**（`.preset-chip` を text-base・太字・ネイビー枠）で最初の行動を明確化。(3) **経済性の案内**：入力 ②③ の上に `.econ-note`「事業の標準値が自動で入る／自社実績に上書きOK」を追加。(4) **目安ヘルパー（bench）を入力カード内（③の直後）へ移動**し、STEP1↔STEP2 の導線をすっきり。(5) **構造バグ修正**：用途（顧客獲得⇄採用）を内容保持で切替えた際、別用途の事業選択が残って誤った前提で判定される問題を、`setUseCase` で「新用途に無い事業なら anchor を解除」して修正。(6) **事業選択時は ⚠目安マークを付けない**方針に統一（自動補完ではなく意図的な選択のため。基準バー＋econ-note で「標準値・調整可」を案内）。import 由来の自動仮置きのみ ⚠ を継続。(7) **用語の平易化（ROAS/ROI/CPA）**：各 KPI の sub を平易な言葉に（ROI「使った費用に対する利益率（＋で黒字）」、CPA「獲得1件あたりにかかる費用」）。結果カードに `.glossary`（`<details>`）で ROAS／ROI／CPA／損益分岐ROAS の意味・式・例を初心者向けに解説。

**稟議文生成＆編集導線の明確化（S093）:** (1) **稟議・申請用テキスト生成**：結果カードに「📝 稟議文を作成」（`ringi-btn`）。`buildRingiText()` が compute() ＋状態から、件名・前置き（黒字＝承認依頼／赤字＝再検討）・試算サマリ（予算/その他固定費/総コスト/想定流入/成約数/売上/粗利/ROAS/ROI/CPA/差引利益）・前提（事業の標準値・資料名）・日付を整形し、`#ringi-text`（編集可 textarea）に出力。`ringi-copy` でコピー、`ringi-close` で閉じる。事業未選択はトーストでガード、印刷時は `.ringi-panel` 非表示。(2) **編集できることの明確化**：②③ は事業選択・資料取込後でも直接書き換え可（live 反映）だが分かりづらかったため、`econ-note` を「✏️ そのまま書き換えOK・右に即反映（反映ボタン不要）」に強化、ガイド STEP3 にも追記。`markManualEdit` で結果カードを淡くパルス（`.just-updated`／`flashResult()`）させ“反映”を可視化。(3) ④その他コストはプリセット読込でも自動展開しない（見出しに「計上 ¥…」を表示）。入力ボックスは全幅でスライダー/トグルと端をそろえる。

**プラン比較の項目拡充＆経済性編集の連動（S094）:** (1) `renderCompare()` の行を拡充：**予算・流入（クリック/リード数）・成約数(CV)・客単価・(購入回数)・(その他固定費)・想定売上(LTV込)・想定粗利・CPA・ROAS・ROI・利益・判定**。展示会の複数ブース比較で必須の「流入＝リード数」が欠けていたのを追加。ラベルは `UC[useCase]` 追従（採用＝応募・接触数/採用数/採用単価/月次粗利、予算は事業非依存で「予算」表記に統一）。(2) **経済性編集の比較連動**：`markManualEdit` で編集フィールドが `ECON_SYNC_FIELDS`（cvr/aov/margin/repeat/othercost＋スライダー）かつ比較プランがあれば `syncScenarioEconomics()` を呼び、各プランの経済性を現フォーム値に揃える（予算・流入はプランごとに維持）。(3) `.preset-note` の負の top margin がチップ拡大後に重なっていた表示崩れを通常 margin に修正。比較表の行ラベル列は `position:sticky; left:0` で横スクロール時も固定。回帰テストは 132 アサーション。

**非マーケ向けの大幅シンプル化（S095）:** 「マーケが分かる人しか使えない」という声を受け、入力画面を「触る所」と「自動の所」に分離。成約率・客単価・粗利率・LTV（＝事業選択で自動充填される経済性）を既定で折りたたみ `<details class="econ-details">`（summary「⚙️ 事業の経済性…マーケに不慣れな方は触らなくてOK」）に格納し、可視部分は「予算・流入（あなたが入れるのはここだけでOK）」のみに。`②③` のセクション番号と `t-sec2` を廃し、`④`/`①` の番号も外して「💰 その他コスト・人件費」「予算・流入」に。bench はその中に内包（details入れ子）。プリセット説明文（`UC.note`）と採用ノートを一文に短縮し、`.preset-note` の負 margin 重なりも解消。結果操作の4ボタン（稟議/PDF/コピー/リセット）は `flex-wrap` の 3+1 ズレを `display:grid; 1fr 1fr` の2×2に。`econAnchored`／判定ゲート／編集の比較連動は維持（折りたたみ内で編集しても live 反映）。

**初見向け情報整理＋用語平易化（S098）:** 「ごちゃごちゃして分かりにくい・初見の人が使えない」「『経済性』という言葉が分からない」という指摘に対応。(1) **用語平易化**：ユーザー向け文言の「経済性」をすべて「前提値（成約率・客単価など）」に置換（basis-bar・折りたたみ見出し・トースト・AI所見・インポート案内）。コード内コメント/変数名（econAnchored 等）は不変。(2) **段階的開示（progressive disclosure）**：結果カードで常時表示していた「損益分岐バー＋ファネル内訳（クリック→CV→売上→粗利→総コスト→利益の7行）」を `<details class="result-breakdown">`（既定で畳む・summary「📊 計算の内訳を見る」）に格納。初見ユーザーが最初に見るのは「判定（黒字/赤字）＋ひとこと説明（result-summary）＋3指標（ROAS/ROI/CPA）」だけになり、未入力時の空の「—」明細も隠れて空状態がすっきり。用語集・詳しい指標は従来どおり `<details>`。(3) **印刷整合**：内訳は PDF/印刷では `display:block !important` で常に展開（明細を残す）。また左カラム再構成（S097）に伴い print の入力非表示セレクタを `.sim-layout > .step` → `.sim-col-left` に修正（入力カードが印刷に出る回帰を防止）。回帰テストの basis-bar 文言チェックを「前提値」に更新し 143 アサーション維持。

**比較表のインライン編集＋レイアウト整理（S097）:** 「画面がごちゃごちゃで見にくい／使いにくい」「サンプルPDFのリード数読み取りが甘い（リストスポンサー以外間違い）」「表も修正できるようにしてほしい・ただし直せる所と直せない所が分かるように」という指摘に対応。(1) **レイアウト再構成**：実機スクショで判明した「左カラム下の巨大な空白＋STEP2の分断（手入力カードは2カラム内・資料インポートは上の全幅バナー）」を解消。`.sim-layout` 直下に `.sim-col-left`（flex縦積み）を新設し、**左カラム＝予算・流入の手入力カード＋資料インポート、右カラム＝結果**に統一（STEP2「予算・流入を入れる」を1か所に集約）。これで左右の高さが釣り合い空白が消え、動線も一本化。`.sim-layout > .step` セレクタは `.sim-col-left > .step` に追従。(2) **クラッター削減**：ガイド3ステップ・STEP1説明・STEP2説明・import-desc・注意書きを大幅短縮。(3) **ブランド準拠**：design-rules.md 違反だった損益バーの角丸（`border-radius:7px`）を除去しシャープな長方形に。(4) **比較表のインライン編集**：`renderCompare` の入力ドライバ行（予算・流入/リード数・客単価・購入回数・その他固定費）を `<input class="cmp-input" data-i data-k>` の編集可能セルにし、`change`（blur/Enter）で `scenarios[i][k]` を更新→再計算。計算結果（成約数CV・売上・粗利・CPA・ROAS・ROI・利益・判定）は `.cell-auto`（グレー・input無し）で**編集不可と視覚区別**し、凡例「編集可＝入力値／灰色＝計算結果」を compare-hint に明示。clicks 編集時は mode を clicks に切替、編集列をアクティブ表示。CPC モードの流入は budget/cpc 由来のため「自動」表示で非編集。これにより AI 読み取りが不正確でもユーザーがその場で各プランの数値を修正できる（リード数取り違え対策）。(5) **抽出プロンプト補強**：出展プランごとの「提供リード数／保証リード数」が表にある場合、各プランの clicks にその行の数値を正確に対応付け、行・列・プラン名の取り違えを避け、対応が不明瞭なら推測流用せず null＋assumptions 明記、と指示を追加。回帰テストは編集可/不可セルの判定・セル編集の反映・他列据え置きを追加し 143 アサーション。

**画像/スクショ・画像PDFの vision 読み取り（S096）:** 「予算・料金の数字がPDFの中にスクショ画像として貼ってある」「スクショ画像はあるが従来は読めない」という課題に対応。`/api/generate`（Anthropic 透過プロキシ）が vision の image コンテンツブロックをそのまま通すことを確認のうえ、`analyzeDocument` をマルチモーダル対応に再設計。(1) `callAI(content, tokens)` ヘルパーに fetch を集約（content はテキスト文字列 or `[{type:'text'},{type:'image',source:{base64}}]` のパート配列）。(2) **画像ファイル**（PNG/JPG/JPEG/WEBP/GIF）は `isImageFile` で判定し、`readImagePart`（FileReader→base64）で vision に直接投入。`ACCEPTED_EXTS`・`<input accept>`・import-desc も画像対応に更新。(3) **PDFフォールバック**：まずテキスト抽出→AI、`hasData(parsed)`（budget/cpc/clicks のいずれか取得）が偽なら `renderPdfToImages(file,6)`（PDF.js で各ページを最大幅1568pxの JPEG 化）で vision 再読み取りし、数値が取れた方を採用（画像化失敗時はテキスト結果にフォールバック）。(4) `usedVision` を追跡し、0件時のエラーメッセージを「AIが画像/PDFを読み取ったが数値を特定できず」と切り分け。テキスト抽出パス（既存のモック fetch テスト）は完全に温存。回帰テストは画像 vision パス（マルチモーダル配列・image パート・値反映）を追加し 138 アサーション。
