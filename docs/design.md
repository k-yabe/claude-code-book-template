...
最終更新: 2026-04-01（Prompt Maker v10 総合UI見直し・機能強化・Wireframe Maker V10）
...
| 2026-04-01 | Wireframe Maker V10 テキスト編集・保存 | SVGプレビュー内テキストのダブルクリック直接編集（全セクションタイプ対応・sec.textsに永続化）、プロジェクト保存/読込（localStorage+JSON出力/読込）、自動保存（デバウンス3秒）、起動時最終プロジェクト自動復元 |
