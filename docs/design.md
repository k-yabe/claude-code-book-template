...
**プリセット:** AKKODiS 公式4事業ライン（コンサルティング／ソリューション／エンジニア派遣／アカデミー）＋エンジニア採用2種の一般的な目安値。結果はテキストでクリップボードコピー可能。
...
**実運用検証（リグレッションテスト）:** `tests/roi-roas-simulator.e2e.js`（`npm run test:roi`）。jsdom で実 DOM ＋ inline script を動かし、初期描画・リアルタイム再計算・プリセット・用途トグル（採用モードのラベル差し替え・スライダー上限）・詳細指標バッジ・業界平均の目安（粗利率チップ・商談化率×受注率→CVR）・人件費概算・流入切替・プラン比較・コピー・UI/UX（はじめ方ガイド・結果見出し・aria）を 45 アサーションで検証（ブラウザ不要・script エラーゼロを確認済み）。AI インポートは既存 `/api/generate`（プロキシ・本番稼働中）と同一契約（model=claude-haiku-4-5・max_tokens 1536＜上限4096・応答 `content[0].text`）なので本番でも動作する。
...
- **AI 連携**: 抽出 JSON に `otherCost` `purchaseCount` `scenarios[]`（label＋差分パラメータ＋note、最大5件）を追加。`scenarioFromExtracted(sc, base)` で「適用後のフォーム値（base）＋プラン差分（非null項目のみ上書き）」として継承生成。資料に2件以上のプランがあれば自動で比較表に展開し先頭を反映。`max_tokens` は 1024→1536。
