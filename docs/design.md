...
最終更新: 2026-04-08（Cache Checker UX改善: キャンセル・進捗ステージ・アクセシビリティ）
...
| 2026-04-09 | Banner Resizer OGP PNG固定・100KB対応(S059) | OGP上限を300KB→100KBに変更（AKKODiS基準準拠）。JPEGフォールバックを廃止し常にPNG出力。100KB超過時はポスタライズ（色数削減7→4bit/ch）＋段階的縮小（100%→60%）の組み合わせで自動最適化。Lossy形式の圧縮品質ステップに0.75/0.70を追加 |
