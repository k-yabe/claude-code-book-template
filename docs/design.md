...
**初心者向け仕上げ＆構造レビュー（S092）:** (1) **レイアウト整列**：ガイド・STEP・入力/結果の幅が不揃い（一部880px・一部コンテナ幅）で右端がガタついていた不具合を、全ブロック同一幅（`.container` 1080px いっぱい）に統一して解消。(2) **事業選択ボタンを大型化**（`.preset-chip` を text-base・太字・ネイビー枠）で最初の行動を明確化。(3) **経済性の案内**：入力 ②③ の上に `.econ-note`「事業の標準値が自動で入る／自社実績に上書きOK」を追加。(4) **目安ヘルパー（bench）を入力カード内（③の直後）へ移動**し、STEP1↔STEP2 の導線をすっきり。(5) **構造バグ修正**：用途（顧客獲得⇄採用）を内容保持で切替えた際、別用途の事業選択が残って誤った前提で判定される問題を、`setUseCase` で「新用途に無い事業なら anchor を解除」して修正。(6) **事業選択時は ⚠目安マークを付けない**方針に統一（自動補完ではなく意図的な選択のため。基準バー＋econ-note で「標準値・調整可」を案内）。import 由来の自動仮置きのみ ⚠ を継続。(7) **用語の平易化（ROAS/ROI/CPA）**：各 KPI の sub を平易な言葉に（ROI「使った費用に対する利益率（＋で黒字）」、CPA「獲得1件あたりにかかる費用」）。結果カードに `.glossary`（`<details>`）で ROAS／ROI／CPA／損益分岐ROAS の意味・式・例を初心者向けに解説。回帰テストは 118 アサーション。
