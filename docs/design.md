...
| 総コスト | `広告費 + その他固定費（人件費等）` |
| 利益 | `粗利 − 総コスト` |
| ROAS | `売上 ÷ 広告費 × 100`（%・標準定義） |
| ROI | `(粗利 − 総コスト) ÷ 総コスト × 100`（%） |
| CPA | `広告費 ÷ CV数` |
| 損益分岐 ROAS | `総コスト ÷ 粗利率 ÷ 広告費 × 100`（粗利で総コストを回収できる売上倍率。固定費0なら `1÷粗利率`） |

**判定:** 利益 ≥ 0 で🟢黒字、< 0 で🔴赤字。赤字時は `黒字化に必要な CVR = 総コスト ÷ (クリック数 × 客単価 × 購入回数 × 粗利率)` を逆算して提示。損益分岐 ROAS をバーの 60% 位置に固定し、実 ROAS をその比率で描画。
