...
| 2026-04-09 | Banner Resizer OGP PNG固定・100KB対応(S059) | OGP上限を300KB→100KBに変更（AKKODiS基準準拠）。JPEGフォールバックを廃止し常にPNG出力。100KB超過時は色数削減（ポスタライズ: 6→5→4→3 bit/ch）で自動圧縮。Lossy形式の圧縮品質ステップに0.75/0.70を追加 |
