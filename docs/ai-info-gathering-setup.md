...
|------|:---:|:----------------:|:-------------------:|
| `/last30days`（**プラグイン由来のスラッシュコマンド**・オリジナル） | ✓ | **✗ CLI 専用** | **✗ CLI 専用** |
| `/last30`（**カスタムスラッシュコマンド**・本リポ製） | ✓ | ✓（要検証） | ✓（要検証） |
| `last30` スキル（自然言語で「last30 スキルで〜」と頼む・本リポ製） | ✓ | ✓ | ✓ |
| `daily-watch` スキル（自然言語で「daily-watch スキルを使って」と頼む） | ✓ | ✓ | ✓ |
| 毎朝 07:00 の launchd 自動実行（`claude -p` 経由） | ✓（実体） | 出力ファイルだけ閲覧可 | 出力ファイルだけ閲覧可 |
| `~/Documents/AI_Daily/*.md` を読んで回答（CLAUDE.md ルール） | ✓ | ✓ | ✓ |

### `/last30days` のデスクトップ対応について

公式の `mvanhorn/last30days-skill` プラグインは Claude Code の **プラグイン由来スラッシュコマンド** で実装されているため、CLI（ターミナル内の `claude`）でしか認識されません。これは Claude Code の仕様。

これを補うため、本リポジトリは bootstrap.sh で **2 つの代替** を同時に install します:

1. **`/last30` カスタムスラッシュコマンド** (`~/.claude/commands/last30.md`)
   - WebSearch + WebFetch で Reddit / HN / GitHub / 公式発表を 30 日横断
   - **CLI では確実に動作**（カスタムコマンドは公式仕様）
   - **デスクトップアプリでの動作は未公式**（公式ドキュメントに記載なし、要実機確認）
   - 使用例: `claude > /last30 Claude Code skills`

2. **`last30` スキル** (`~/.claude/skills/last30/SKILL.md`)
   - 上と同じロジックを **スキル形式** で実装
   - スキルは仕様上すべての入口から自然言語で呼び出し可能
   - 使用例（デスクトップアプリで）: 「last30 スキルで Claude Agent SDK の30日トレンドを調べて」

**推奨運用**: まずデスクトップアプリで `/last30 <query>` を試し、認識されたらそれを使う。認識されなければ「last30 スキルで <query>」と自然言語で頼めば確実に動く。

**重要な実用面**: 加えて、デスクトップアプリで AI 動向を質問すると、CLAUDE.md のナレッジ参照ルールが効いて `~/Documents/AI_Daily/*.md` を自動で読みに行く設計になっています。`/last30` も `/last30days` も叩かずに、ただ「最近の Claude Code どう？」と聞くだけで蓄積された情報が文脈に効きます。
