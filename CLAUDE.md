...
| バッジ | 使う場面 | 配色（カード／WHAT'S NEW 共通） |
|-------|---------|--------------------------------|
| `NEW` | 新規アプリの公開、または既存アプリへの**新機能**追加 | Navy背景 (#001f33) + Gold文字 (#ffb81c) |
| `UPDATE` | 既存機能の改善・バグ修正・UX改善・リファクタ・アクセシビリティ対応など（ほとんどはこちら） | Gold背景 (#ffb81c) + Navy文字 (#001f33) |

迷ったら `UPDATE`。`NEW` は「ユーザーが初めて触れる何か」が増えた時のみ。

**色の運用**: カード (`.card-new-badge` / `.card-update-badge`) と WHAT'S NEW (`.cl-badge.new` / `.cl-badge.update`) は **必ず同じ配色** にする（NEW=ダーク背景・UPDATE=明るい背景）。片方だけ変更して不整合を生じさせない。
