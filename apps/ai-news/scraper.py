...
_B2C_FLUFF_WORDS = [
    ...
    # 音楽・アニメ・エンタメタイアップ（B2C 文化消費系で AKKODiS には無関係）
    # 例: 「ソニーミュージックがアニメ左ききのエレン主題歌タイアップ」
    "主題歌", "オープニング曲", "エンディング曲", "OP曲", "ED曲",
    "音楽タイアップ", "アニメタイアップ", "コラボ楽曲", "コラボ曲",
    "MVを公開", "ミュージックビデオ公開",
    "アニメ化", "実写化", "ドラマ化", "舞台化",
    "シーズン2", "新シリーズ放送",
...
def call_anthropic(items: list[dict]) -> tuple[list[dict], list[str]] | None:
    ...
    system_prompt = (
        ...
        "**重要 — 取り上げる対象**: AI / 生成AI / B2B マーケティング / 採用マーケ / "
        "競合企業（パーソル・リクルート・SIer・コンサル等）の動向に該当する記事のみ。\n"
        "**取り上げない**: B2C 商品（飲料・コーラ・コンビニ新商品）、エンタメ "
        "（アニメ主題歌・音楽タイアップ・芸能人）、グルメ・観光・スポーツ・ファッションの "
        "新商品ニュースは絶対に executive summary に入れない。たとえ urgency=must_know で "
        "拾った記事でも、3行サマリーは AI/B2B/採用 に該当する記事のみで構成する。\n\n"
        "煽りや推測は避け、事実ベースで。\n"
