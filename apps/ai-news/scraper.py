...
def call_anthropic(items: list[dict]) -> list[dict] | None:
    ...
    system_prompt = (
        ...
        "- urgency: must_know（今日中に全員把握、最大2件）/ this_week（今週チェック、最大5件）/ fyi（余裕があれば）\n"
        "- tags: 3〜5個の日本語タグ\n"
        "- importance: 1=最重要1件のみ、2=押さえるべき5件、3=その他\n"
        "- readMin: 推定読了時間（1〜3分）\n\n"
        "## executiveSummary\n"
        "全記事を俯瞰した3項目の箇条書き。各項目は以下の形式:\n"
        "「【期限】具体的アクション → 背景と根拠（1文）」\n"
        "期限は「今日中」「今週中」「今月中」のいずれか。\n"
        "マーケ部門の朝会で読み上げてそのまま行動に移せる粒度にすること。\n\n"
        "煽りや推測は避け、事実ベースで。\n"
