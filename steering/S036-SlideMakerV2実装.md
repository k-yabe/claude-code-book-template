...
- [x] `api/slide-factcheck.js`: 新規作成
  - claude-haiku-4-5 + web_search (max_uses: 3)
  - 入力: 単一スライドの内容
  - 出力: `{ claims: [{ text, verified, source_url }] }`
- [x] `apps/slide-maker/index.html`: 各スライドに「ファクトチェック」ボタン追加
...
### 2026-03-30: Phase 3 ファクトチェック機能実装
- `api/slide-factcheck.js` 新規作成: claude-haiku-4-5 + web_search (max_uses: 3) でスライド単位の主張検証
- `apps/slide-maker/index.html` にファクトチェックUI追加: プレビュー各スライドに「✓ 検証」ボタン、結果オーバーレイ（検証済/要確認/検証不能の3段階表示+出典リンク）
