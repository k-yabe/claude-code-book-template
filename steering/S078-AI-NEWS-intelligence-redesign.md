...
- 2026-04-17: UX大幅強化。スクロール進捗バー（Gold→Cyanグラデーション）、既読プログレス表示、トップへ戻るボタン、画像スケルトンローディング。注目カードに影響/アクションプレビュー表示。カード登場アニメーション（staggered fade-in）。ヒーロー背景グラデーションアニメーション。統計カウントアップ。★ポップアニメーション。記事共有ボタン（Web Share API + clipboard fallback）。トースト通知。「次の未読」「全部既読」ボタン。キーボードショートカット n/m 追加。X引用風デザイン。
