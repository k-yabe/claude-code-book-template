...
> AIエージェントがUI・資料・スライドを作成する際は必ずこのファイルを参照すること。
...
### ルール
- **NEW バッジ**: Navy背景 `#001f33` + Gold文字 `#ffb81c` — 新機能・新規公開
- **UPDATE バッジ**: Gold背景 `#ffb81c` + Navy文字 `#001f33` — 改善・バグ修正
...
このデザインシステムを使ってUIを作成すること:
1. カラーパレットから外れた色を使わない
2. フォントスタックを守る（日本語フォントを必ず含める）
3. バッジのNEW/UPDATEルールを厳守
4. 過度な装飾より可読性・使いやすさを優先
