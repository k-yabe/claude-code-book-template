...
MacBook Air M3（常時起動）でセットアップ済みの場合、**1時間ごとに未着手タスクをAIが自動処理**します。

```mermaid
sequenceDiagram
    participant LD as ⏰ launchd（1時間ごと）
...
- タスクボードの 🚨 要確認 カラムにタスクが移動
- カードを開いてコメントを読み → 必要な判断・情報をコメントに追記
- AIが次回実行時に続きを処理
...
MacBook Air M3 で設定済みの場合、**毎朝7時に自動でデイリーレポートが生成**されます。
