...
| layout | テンプレート | 使用条件 |
|--------|------------|---------|
| text-left-picture-right | テキスト左+画像右 | 概念説明+イメージ写真 |
| text-right-picture-left | テキスト右+画像左 | 上記の交互配置用 |
| content-left-full-picture-right | テキスト左+大画像右 | インパクトのある画像がある場合 |
| large-image-right | テキスト+大画像 | ビジュアル重視の説明 |
| picture-fullscreen | フルスクリーン画像 | セクション間のビジュアルブレイク |
` : '';

  const imageFields = imageEnabled ? `
text-left-picture-right:        { layout:"text-left-picture-right", title, body, imageQuery, notes }
text-right-picture-left:        { layout:"text-right-picture-left", title, body, imageQuery, notes }
content-left-full-picture-right:{ layout:"content-left-full-picture-right", title, body, imageQuery, notes }
large-image-right:              { layout:"large-image-right", title, body, imageQuery, notes }
picture-fullscreen:             { layout:"picture-fullscreen", title, imageQuery, notes }` : '';

  const imageQuerySection = imageEnabled ? `
### imageQuery（画像系レイアウト必須）
- 英語で記述（Unsplash API用）。例: "digital transformation office", "team collaboration meeting"
- 具体的で写真が見つかりやすいキーワード（抽象的すぎる語は避ける）
- スライドの内容に関連する高品質なストック写真をイメージして記述
` : '';

  const layoutRule = imageEnabled
    ? `  - テキスト系（content, two-column, centered-text）: **20%以下**
  - 画像系（text-left-picture-right等）: **25%以上**
  - データ・図解系（content-with-chart, content-with-flow, sixbox, comparison, quote）: **25%以上**`
    : `  - テキスト系（content, two-column, centered-text）: **30%以下**
  - データ・図解系（content-with-chart, content-with-flow, sixbox, comparison, quote）: **40%以上**`;

  return `あなたはAKKODiSのプレゼンテーション構成の専門家です。
...
### テキスト系レイアウト（全体の20%以下に抑える）
| layout | テンプレート | 使用条件 |
|--------|------------|---------|
| content | 1段テキスト | テキスト主体（最小限に） |
| two-column | 2段テキスト | 左右で比較説明、メリット/デメリット |
| centered-text | 中央寄せテキスト | キーメッセージの強調、1文で伝えたい主張 |
${imageSection}
### データ・図解系レイアウト（積極的に活用すること）
| layout | テンプレート | 使用条件 |
|--------|------------|---------|
| content-with-chart | テキスト+グラフ | 数値・データ・推移・比較数値（棒/折線/円） |
| content-with-flow | テキスト+フロー図 | プロセス・手順・ステップ |
| sixbox | 6ボックス | 6要素の並列表示（サービス、強み等） |
| comparison | テーブル比較 | 2択比較・現状vs改善 |
| quote | 引用・顧客の声 | 顧客コメント、キーメッセージ |

**最重要ルール — 図版・ビジュアル優先**:
- プレゼンは「見て分かる」が原則。文字だらけのスライドは禁止
- **レイアウト配分ルール**（cover/agenda/chapter/closingを除くコンテンツスライド）:
${layoutRule}
...
centered-text:                  { layout:"centered-text", title, body, notes }
${imageFields}
...
- centered-text: { title, body, notes }
- text-left-picture-right: { title, body, imageQuery, notes }
- text-right-picture-left: { title, body, imageQuery, notes }
- content-left-full-picture-right: { title, body, imageQuery, notes }
...
- 補足データがある場合は必ず content-with-chart または comparison を1枚以上使う`;
    ...
    const validLayouts = ['cover', 'agenda', 'chapter', 'content', 'two-column', 'centered-text', 'text-left-picture-right', 'text-right-picture-left', 'content-left-full-picture-right', 'large-image-right', 'picture-fullscreen', 'content-with-chart', 'content-with-flow', 'sixbox', 'comparison', 'quote', 'closing'];
