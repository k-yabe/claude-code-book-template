...
| content-with-image | テキスト+画像 | 写真・図解の説明文付き（※画像はチャートPNGとして生成） |
| content-with-chart | テキスト+グラフ | ユーザーが数値・データ・推移・比較数値を提供した場合 |
...
two-column: { layout, title, leftBody, rightBody, notes }
content-with-image: { layout, title, body, chart: { type, title, labels[], data[], unit }, notes }
...
sixbox:  { layout, title, boxes: [{ heading, body }] (6個), notes }
comparison: { layout, title, table: { headers[], rows[][] }, notes }
quote:   { layout, speaker, body, notes }
...
- content-with-image: { title, body, chart: { type, title, labels[], data[], unit }, notes }
...
- 補足データがある場合は必ず content-with-chart または comparison を1枚以上使う`;
    ...
    const validLayouts = ['cover', 'agenda', 'chapter', 'content', 'two-column', 'content-with-image', 'content-with-chart', 'content-with-flow', 'sixbox', 'comparison', 'quote', 'closing'];
